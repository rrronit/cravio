...
export async function getRecipe(c: AppContext) {
  return c.json(await createServices(c.env.DB).recipes.get(c.req.param('id')!, currentUserId(c)));
}
...
export async function updateRecipe(c: AppContext) {
  ...
  return c.json(await createServices(c.env.DB).recipes.update(c.req.param('id')!, currentUserId(c), input.data));
}

export async function removeRecipe(c: AppContext) {
  await createServices(c.env.DB).recipes.delete(c.req.param('id')!, currentUserId(c));
