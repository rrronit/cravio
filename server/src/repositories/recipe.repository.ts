...
export const createRecipeRepository = (db: D1Database) => ({
  findAll: async (userId: string): Promise<Recipe[]> => {
    const { results } = await db.prepare('SELECT * FROM recipes WHERE user_id = ? ORDER BY imported_at DESC').bind(userId).all<RecipeRow>();
    return results.map(mapRecipeRow);
  },
  findById: async (id: string, userId: string): Promise<Recipe | null> => {
    const row = await db.prepare('SELECT * FROM recipes WHERE id = ? AND user_id = ?').bind(id, userId).first<RecipeRow>();
    return row ? mapRecipeRow(row) : null;
  },
  findTemplate: async (): Promise<Recipe | null> => {
    const row = await db.prepare('SELECT * FROM recipes ORDER BY imported_at LIMIT 1').first<RecipeRow>();
    return row ? mapRecipeRow(row) : null;
  },
  ...
  delete: async (id: string, userId: string): Promise<void> => {
    await db.prepare('DELETE FROM recipes WHERE id = ? AND user_id = ?').bind(id, userId).run();
  },
});
...
const mapRecipeRow = (row: RecipeRow): Recipe => ({
  id: row.id, userId: row.user_id, title: row.title, description: row.description, creator: row.creator,
  platform: row.platform, sourceUrl: row.source_url, image: row.image, prepTime: row.prep_time,
  cookTime: row.cook_time, servings: row.servings, cuisine: row.cuisine, difficulty: row.difficulty,
  tags: parseJson(row.tags_json, []), ingredients: parseJson(row.ingredients_json, []),
  ...
  favorite: Boolean(row.favorite), confidence: row.confidence, warnings: parseJson(row.warnings_json, []), importedAt: row.imported_at,
});
