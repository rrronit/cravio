...
export class RecommendationService {
  ...
  async list(userId: string, category?: RecommendationCategory): Promise<Recommendation[]> {
    const [recipes, pantry] = await Promise.all([this.recipes.findAll(userId), this.pantry.findAll(true)]);
    const owned = new Set(pantry.map((item) => normalizeIngredient(item.name)));
    return recipes.map((recipe): Recommendation => {
      ...
      const recommendationCategory: RecommendationCategory = match === 100
        ? 'can_make_now'
        : missing.length <= 2
          ? 'almost_ready'
          : match >= 50 ? 'best_matches' : 'need_shopping';
