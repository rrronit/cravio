...
export const createIngredientService = (recipes: RecipeRepository) => ({
  list: async (userId: string) => {
    const counts = new Map<string, { name: string; recipeIds: string[] }>();
    for (const recipe of await recipes.findAll(userId)) {
    ...
    return [...counts.values()].map((item) => ({ ...item, recipeCount: item.recipeIds.length })).sort((a, b) => b.recipeCount - a.recipeCount);
  },
});
