...
import {
  Alert, BackHandler, FlatList, Image, ImageBackground, KeyboardAvoidingView, Linking, Modal, Platform, Pressable,
...
function BrandHeader({ title, subtitle, back, onBack }: { title?: string; subtitle?: string; back?: boolean; onBack?: () => void }) {
  return (
    <View style={styles.header}>
      ...
      <View style={{ flex: 1 }}>
        <Text style={title ? styles.headerTitle : styles.brand}>{title ?? 'cravio'}</Text>
        ...
        <Pressable style={styles.importHero} onPress={() => onTab('Import')}>
          <LinearGradient colors={['#1D6045', '#163D2F']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.importGradient}>
            <View style={styles.importCopy}>
              ...
              <Text style={styles.importTitle}>Found something{`\n`}delicious?</Text>
              <Text style={styles.importText}>Paste a reel. We'll turn it into a recipe.</Text>
              <View style={styles.importButton}><Feather name="link-2" size={17} color={colors.green} /><Text style={styles.importButtonText}>Import a recipe</Text></View>
            </View>
            <View style={styles.heroArt}><Text style={styles.heroEmoji}>🍝</Text><View style={styles.sparkle}><Ionicons name="sparkles" size={20} color={colors.green} /></View></View>
          </LinearGradient>
        </Pressable>
  ...
  return (
    <View style={styles.flexScreen}><ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.chipRow}>
        {filters.map((item) => <Pressable key={item} onPress={() => setFilter(item)} style={[styles.chip, filter === item && styles.chipActive]}><Text style={[styles.chipText, filter === item && styles.chipTextActive]}>{item}</Text></Pressable>)}
      </ScrollView>
      <View style={styles.page}>
...
function ImportScreen({ onComplete }: { onComplete: (recipe: Recipe) => void }) {
  ...
  const [status, setStatus] = useState<'idle' | 'extracting' | 'generating' | 'ready'>('idle');
  const timers = useRef<ReturnType<typeof setTimeout>[]>([]);
  useEffect(() => () => timers.current.forEach(clearTimeout), []);
  const startImport = () => {
    if (!url.trim()) return Alert.alert('Paste a recipe link', 'Add an Instagram, TikTok, or YouTube link first.');
    setStatus('extracting'); timers.current = [setTimeout(() => setStatus('generating'), 900), setTimeout(() => setStatus('ready'), 1900)];
  };
  const saveImport = () => {
    const imported: Recipe = { ...recipeSeed[1], id: `${Date.now()}`, title: 'Smoky Harissa Paneer', creator: '@yourlatestsave', sourceUrl: url, favorite: false, match: 80, missing: ['Harissa paste'] };
    onComplete(imported); setUrl(''); setStatus('idle');
  };
  return (
    <KeyboardAvoidingView style={styles.flexScreen} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}><View style={styles.page}><BrandHeader />
        ...
        <View style={styles.urlCard}><Text style={styles.inputLabel}>RECIPE VIDEO LINK</Text><View style={styles.urlInput}><Feather name="link-2" size={20} color={colors.muted} /><TextInput value={url} onChangeText={setUrl} autoCapitalize="none" keyboardType="url" placeholder="https://instagram.com/reel/..." placeholderTextColor="#A2AAA5" style={styles.urlTextInput} /></View>
          <Pressable style={[styles.primaryButton, status !== 'idle' && status !== 'ready' && { opacity: 0.7 }]} onPress={startImport} disabled={status !== 'idle'}><Ionicons name="sparkles" size={18} color={colors.lime} /><Text style={styles.primaryButtonText}>Create my recipe</Text><Feather name="arrow-right" size={18} color={colors.surface} /></Pressable>
        </View>
        {status !== 'idle' ? <View style={styles.processCard}>
          <View style={styles.processTop}><Text style={styles.processTitle}>{status === 'ready' ? 'Your recipe is ready!' : 'Creating your recipe'}</Text><Text style={styles.processPercent}>{status === 'extracting' ? '35%' : status === 'generating' ? '72%' : '100%'}</Text></View>
          <View style={styles.progressTrack}><View style={[styles.progressBar, { width: status === 'extracting' ? '35%' : status === 'generating' ? '72%' : '100%' }]} /></View>
          {['Reading the video', 'Building ingredients & steps', 'Estimating nutrition'].map((label, index) => { const complete = status === 'ready' || (status === 'generating' && index === 0); return <View key={label} style={styles.processStep}>{complete ? <Ionicons name="checkmark-circle" size={20} color={colors.green} /> : <View style={[styles.stepDot, index === (status === 'extracting' ? 0 : 1) && styles.stepDotActive]} />}<Text style={[styles.processStepText, complete && { color: colors.ink }]}>{label}</Text></View>; })}
          {status === 'ready' ? <Pressable style={styles.saveButton} onPress={saveImport}><Text style={styles.saveButtonText}>Review & save recipe</Text><Feather name="arrow-right" size={18} color={colors.green} /></Pressable> : null}
        </View> : null}
...
function PantryScreen({ pantry, onAdd }: { pantry: PantryItem[]; onAdd: () => void }) {
  ...
  return <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}><View style={styles.page}><BrandHeader /><View style={styles.titleAction}><View><Text style={styles.screenTitle}>My pantry</Text><Text style={styles.screenSubtitle}>{pantry.length} ingredients on hand.</Text></View><Pressable style={styles.roundAdd} onPress={onAdd}><Feather name="plus" size={23} color={colors.surface} /></Pressable></View>
    <View style={{ marginTop: 20 }}><SearchBar value={query} onChangeText={setQuery} placeholder="Find an ingredient" /></View>
    <View style={styles.pantryHero}><View style={{ flex: 1 }}><Text style={styles.pantryHeroLabel}>PANTRY POWER</Text><Text style={styles.pantryHeroTitle}>You can make{`\n`}2 recipes right now</Text><Text style={styles.pantryHeroText}>And 3 more with just one ingredient.</Text></View><View style={styles.pantryScore}><Text style={styles.pantryScoreNumber}>78</Text><Text style={styles.pantryScoreLabel}>PANTRY{`\n`}SCORE</Text></View></View>
...
function ProfileScreen({ recipes, pantryCount }: { recipes: Recipe[]; pantryCount: number }) {
  ...
  return <ScrollView contentContainerStyle={styles.scroll}><View style={styles.page}><BrandHeader /><View style={styles.profileTop}><View style={styles.profileAvatar}><Text style={styles.profileInitial}>R</Text></View><Text style={styles.profileName}>Ronit</Text><Text style={styles.profileEmail}>ronit@cravio.app</Text><View style={styles.memberBadge}><Ionicons name="sparkles" size={13} color={colors.green} /><Text style={styles.memberText}>CRAVIO EARLY TASTER</Text></View></View>
...
export default function App() {
  const [tab, setTab] = useState<TabName>('Home'); const [recipeList, setRecipeList] = useState(recipeSeed); const [pantry, setPantry] = useState(pantrySeed); const [detail, setDetail] = useState<Recipe | null>(null); const [addPantry, setAddPantry] = useState(false);
  ...
  return <SafeAreaProvider><StatusBar style={detail ? 'light' : 'dark'} />
    {detail ? <RecipeDetail recipe={detail} onBack={() => setDetail(null)} onFavorite={() => favorite(detail.id)} /> : <SafeAreaView style={styles.app} edges={['top']}>
      ...
      {tab === 'Profile' && <ProfileScreen recipes={recipeList} pantryCount={pantry.length} />}
...
const styles = StyleSheet.create({
  app: { flex: 1, backgroundColor: colors.background }, flexScreen: { flex: 1 }, detailScreen: { flex: 1, backgroundColor: colors.surface }, scroll: { paddingBottom: 115 }, page: { ...contentWidth, paddingHorizontal: 20 },
  header: { height: 58, flexDirection: 'row', alignItems: 'center', gap: 11 }, logo: { width: 34, height: 34, borderRadius: 12, backgroundColor: colors.green, alignItems: 'center', justifyContent: 'center', transform: [{ rotate: '-8deg' }] }, logoMark: { color: colors.lime, fontSize: 19, fontWeight: '900' }, brand: { color: colors.ink, fontSize: 22, fontWeight: '900', letterSpacing: -0.7 }, headerTitle: { color: colors.ink, fontSize: 17, fontWeight: '800' }, headerSubtitle: { color: colors.muted, fontSize: 11 }, iconButton: { width: 40, height: 40, borderRadius: 14, backgroundColor: colors.surface, alignItems: 'center', justifyContent: 'center', ...shadow }, notifyDot: { position: 'absolute', right: 9, top: 9, width: 7, height: 7, borderRadius: 4, backgroundColor: colors.orange, borderWidth: 1.5, borderColor: colors.surface },
  ...
  screenTitle: { color: colors.ink, fontSize: 34, lineHeight: 39, fontWeight: '900', letterSpacing: -1, marginTop: 20 }, screenSubtitle: { color: colors.muted, fontSize: 14, lineHeight: 20, marginTop: 5 }, chipRow: { paddingHorizontal: 20, paddingVertical: 18, gap: 8 }, chip: { height: 38, borderRadius: 19, paddingHorizontal: 16, backgroundColor: colors.surface, alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: colors.line }, chipActive: { backgroundColor: colors.green, borderColor: colors.green }, chipText: { color: colors.muted, fontSize: 12, fontWeight: '700' }, chipTextActive: { color: colors.surface }, collectionStats: { backgroundColor: colors.greenSoft, padding: 18, borderRadius: 20, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-around', marginBottom: 24 }, statNumber: { textAlign: 'center', fontSize: 20, fontWeight: '900', color: colors.green }, statLabel: { textAlign: 'center', fontSize: 8, letterSpacing: 1, fontWeight: '800', color: colors.muted, marginTop: 2 }, statDivider: { width: 1, height: 28, backgroundColor: '#C3D8CC' }, resultHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }, resultText: { fontSize: 20, fontWeight: '900', color: colors.ink }, sort: { flexDirection: 'row', alignItems: 'center', gap: 5 }, sortText: { fontSize: 11, fontWeight: '700', color: colors.muted }, empty: { alignItems: 'center', paddingVertical: 50 }, emptyEmoji: { fontSize: 42 }, emptyTitle: { color: colors.ink, fontWeight: '900', fontSize: 19, marginTop: 12 }, emptyText: { color: colors.muted, fontSize: 13, marginTop: 5 },
  importPageIcon: { width: 58, height: 58, borderRadius: 20, backgroundColor: colors.greenSoft, alignItems: 'center', justifyContent: 'center', alignSelf: 'center', marginTop: 26 }, urlCard: { marginTop: 26, borderRadius: 24, backgroundColor: colors.surface, padding: 18, ...shadow }, inputLabel: { color: colors.muted, fontSize: 9, fontWeight: '900', letterSpacing: 1.2, marginBottom: 8, marginTop: 5 }, urlInput: { height: 54, borderRadius: 16, borderWidth: 1, borderColor: colors.line, backgroundColor: colors.background, flexDirection: 'row', alignItems: 'center', gap: 10, paddingHorizontal: 14 }, urlTextInput: { flex: 1, height: '100%', color: colors.ink, fontSize: 13 }, primaryButton: { minHeight: 54, marginTop: 14, backgroundColor: colors.green, borderRadius: 17, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 10, paddingHorizontal: 18 }, primaryButtonText: { color: colors.surface, fontSize: 14, fontWeight: '800', flex: 1, textAlign: 'center' }, processCard: { backgroundColor: colors.surface, borderRadius: 22, padding: 18, marginTop: 16, ...shadow }, processTop: { flexDirection: 'row', justifyContent: 'space-between' }, processTitle: { color: colors.ink, fontSize: 15, fontWeight: '800' }, processPercent: { color: colors.green, fontSize: 13, fontWeight: '900' }, progressTrack: { height: 6, borderRadius: 3, backgroundColor: colors.greenSoft, marginVertical: 14, overflow: 'hidden' }, progressBar: { height: 6, backgroundColor: colors.green, borderRadius: 3 }, processStep: { flexDirection: 'row', alignItems: 'center', gap: 10, marginTop: 9 }, stepDot: { width: 18, height: 18, borderRadius: 9, backgroundColor: colors.line }, stepDotActive: { backgroundColor: colors.orange, borderWidth: 5, borderColor: colors.orangeSoft }, processStepText: { color: colors.muted, fontSize: 12, fontWeight: '600' }, saveButton: { height: 48, borderRadius: 15, marginTop: 16, backgroundColor: colors.lime, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 9 }, saveButtonText: { color: colors.green, fontWeight: '800' }, howCard: { marginTop: 24, backgroundColor: colors.surface, borderRadius: 24, padding: 20 }, howTitle: { color: colors.ink, fontSize: 19, fontWeight: '900', marginBottom: 5 }, howRow: { flexDirection: 'row', alignItems: 'center', gap: 13, paddingVertical: 14 }, howNumber: { width: 38, height: 38, borderRadius: 13, backgroundColor: colors.greenSoft, alignItems: 'center', justifyContent: 'center' }, howRowTitle: { color: colors.ink, fontSize: 13, fontWeight: '800' }, howRowText: { color: colors.muted, fontSize: 11, lineHeight: 16, marginTop: 2 }, howLine: { position: 'absolute', width: 1, height: 18, backgroundColor: colors.line, left: 19, bottom: -9 }, platforms: { marginTop: 24 }, platformLabel: { textAlign: 'center', color: colors.muted, fontSize: 9, fontWeight: '900', letterSpacing: 1.3 }, platformRow: { flexDirection: 'row', justifyContent: 'center', gap: 18, marginTop: 12 }, platform: { flexDirection: 'row', alignItems: 'center', gap: 6 }, platformText: { color: colors.ink, fontWeight: '700', fontSize: 11 },
  titleAction: { flexDirection: 'row', alignItems: 'flex-end', justifyContent: 'space-between' }, roundAdd: { width: 48, height: 48, borderRadius: 17, backgroundColor: colors.green, alignItems: 'center', justifyContent: 'center' }, pantryHero: { marginVertical: 25, padding: 22, borderRadius: 24, backgroundColor: colors.green, flexDirection: 'row', alignItems: 'center' }, pantryHeroLabel: { color: colors.lime, fontSize: 9, fontWeight: '900', letterSpacing: 1.3 }, pantryHeroTitle: { color: colors.surface, fontSize: 22, lineHeight: 26, fontWeight: '900', marginTop: 7 }, pantryHeroText: { color: '#BBD0C5', fontSize: 11, marginTop: 7 }, pantryScore: { width: 86, height: 86, borderRadius: 43, borderWidth: 6, borderColor: colors.lime, alignItems: 'center', justifyContent: 'center' }, pantryScoreNumber: { color: colors.surface, fontSize: 23, fontWeight: '900' }, pantryScoreLabel: { textAlign: 'center', color: '#C8D6CE', fontSize: 7, lineHeight: 9, fontWeight: '800' }, pantryItem: { minHeight: 72, padding: 10, borderRadius: 18, backgroundColor: colors.surface, marginBottom: 9, flexDirection: 'row', alignItems: 'center', gap: 12 }, pantryIcon: { width: 50, height: 50, borderRadius: 15, backgroundColor: colors.background, alignItems: 'center', justifyContent: 'center' }, pantryEmoji: { fontSize: 25 }, pantryName: { color: colors.ink, fontSize: 14, fontWeight: '800' }, pantryQty: { color: colors.muted, fontSize: 11, marginTop: 4 }, expiry: { paddingHorizontal: 9, height: 25, borderRadius: 12, backgroundColor: colors.greenSoft, justifyContent: 'center' }, expirySoon: { backgroundColor: colors.orangeSoft }, expiryText: { color: colors.green, fontSize: 9, fontWeight: '800' }, expirySoonText: { color: '#A85F25' }, outlineButton: { height: 52, borderRadius: 17, borderWidth: 1.5, borderStyle: 'dashed', borderColor: '#A6BCAF', marginTop: 8, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 7 }, outlineButtonText: { color: colors.green, fontWeight: '800', fontSize: 13 },
  profileTop: { alignItems: 'center', marginTop: 20 }, profileAvatar: { width: 82, height: 82, borderRadius: 28, backgroundColor: colors.orangeSoft, alignItems: 'center', justifyContent: 'center', borderWidth: 4, borderColor: colors.surface, ...shadow }, profileInitial: { color: colors.green, fontSize: 32, fontWeight: '900' }, profileName: { fontSize: 24, fontWeight: '900', color: colors.ink, marginTop: 12 }, profileEmail: { color: colors.muted, fontSize: 12, marginTop: 3 }, memberBadge: { backgroundColor: colors.lime, paddingHorizontal: 11, height: 26, borderRadius: 13, flexDirection: 'row', alignItems: 'center', gap: 5, marginTop: 10 }, memberText: { color: colors.green, fontSize: 8, letterSpacing: 1, fontWeight: '900' }, profileStats: { flexDirection: 'row', justifyContent: 'space-around', backgroundColor: colors.surface, padding: 20, borderRadius: 22, marginTop: 24, ...shadow }, profileStatNum: { textAlign: 'center', color: colors.ink, fontSize: 21, fontWeight: '900' }, profileStatText: { textAlign: 'center', color: colors.muted, fontSize: 10, marginTop: 3 }, settingsLabel: { color: colors.muted, fontSize: 9, letterSpacing: 1.4, fontWeight: '900', marginTop: 26, marginBottom: 8 }, settingsCard: { backgroundColor: colors.surface, borderRadius: 22, paddingHorizontal: 14 }, settingRow: { minHeight: 68, flexDirection: 'row', alignItems: 'center', gap: 12, borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: colors.line }, settingIcon: { width: 38, height: 38, borderRadius: 13, backgroundColor: colors.greenSoft, alignItems: 'center', justifyContent: 'center' }, settingTitle: { color: colors.ink, fontSize: 13, fontWeight: '800' }, settingSub: { color: colors.muted, fontSize: 10, marginTop: 3 }, version: { color: colors.muted, fontSize: 10, textAlign: 'center', marginVertical: 28 },
