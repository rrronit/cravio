      ...
      <View style={styles.header}>
        {back ? <Pressable style={styles.iconButton} onPress={onBack}><Feather name="arrow-left" size={21} color={colors.ink} /></Pressable> : (
          <View style={styles.brandLogoFrame} accessibilityLabel="Cravio">
            {colors.background === '#0D1511' ? <Text style={styles.darkBrand}>cravio</Text> : <Image source={require('./assets/cravio-logo.png')} style={styles.brandLogo} resizeMode="cover" />}
          </View>
        )}
        <View style={{ flex: 1 }}>
    ...
    <KeyboardAvoidingView style={styles.flexScreen} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}><View style={styles.page}><BrandHeader />
        ...
        <View style={styles.urlCard}><Text style={styles.inputLabel}>RECIPE VIDEO LINK</Text><View style={styles.urlInput}><Feather name="link-2" size={20} color={colors.muted} /><TextInput value={url} onChangeText={setUrl} autoCapitalize="none" keyboardType="url" placeholder="https://instagram.com/reel/..." placeholderTextColor="#A2AAA5" style={styles.urlTextInput} /></View>
          <Pressable style={[styles.primaryButton, status !== 'idle' && status !== 'ready' && { opacity: 0.7 }]} onPress={startImport} disabled={status !== 'idle'}><Ionicons name="sparkles" size={18} color={colors.lime} /><Text style={[styles.primaryButtonText, { color: colors.onPrimary }]}>Create my recipe</Text><Feather name="arrow-right" size={18} color={colors.onPrimary} /></Pressable>
        </View>
...
function ProfileScreen({ recipes, pantryCount, darkMode, onDarkModeChange }: { recipes: Recipe[]; pantryCount: number; darkMode: boolean; onDarkModeChange: (value: boolean) => void }) {
...
export default function App() {
  const [tab, setTab] = useState<TabName>('Home'); const [recipeList, setRecipeList] = useState(recipeSeed); const [pantry, setPantry] = useState(pantrySeed); const [detail, setDetail] = useState<Recipe | null>(null); const [addPantry, setAddPantry] = useState(false); const [darkMode, setDarkMode] = useState(false);
  applyTheme(darkMode);
  styles = createStyles();
  ...
  return <SafeAreaProvider><StatusBar style={detail || darkMode ? 'light' : 'dark'} />
    {detail ? <RecipeDetail recipe={detail} onBack={() => setDetail(null)} onFavorite={() => favorite(detail.id)} /> : <SafeAreaView style={styles.app} edges={['top']}>
      ...
      {tab === 'Profile' && <ProfileScreen recipes={recipeList} pantryCount={pantry.length} darkMode={darkMode} onDarkModeChange={setDarkMode} />}
